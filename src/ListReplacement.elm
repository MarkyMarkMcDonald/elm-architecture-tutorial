...
fromIf shouldReplace {items, source} =
    let result = fromIfHelper shouldReplace { items = items, source = source, output = []} in
...
fromIfHelper shouldReplace { items, source, output } =
    let replaceFrom = fromIfHelper shouldReplace in
    case items of
      ...
      inputItem :: rest ->
          if shouldReplace inputItem then
              case source of
                  ...
                  sourceItem :: sourceTail -> replaceFrom { items = rest, source = sourceTail, output = output ++ [sourceItem] }
         else
